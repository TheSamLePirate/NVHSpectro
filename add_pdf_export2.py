    ...
    fun generatePdfReport(context: Context) {
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                ...
                if (kin.isEnabled) {
                    canvas.drawText("Vehicule: \", 50f, 95f, paintText)
                    canvas.drawText("Moteur: \", 50f, 110f, paintText)
                    canvas.drawText("V1000 equivalente: \ km/h", 50f, 125f, paintText)
                }
                
                val orders = _manualTrackedOrders.value
                var yPos = 160f
                canvas.drawText("Ordres Identifies:", 50f, yPos, paintTitle)
                yPos += 20f
                
                val headers = if (kin.isEnabled) "Ordre | Plage RPM | Plage Vitesse (km/h) | Plage Freq (Hz) | Max Emergence (TTNR)" 
                              else "Ordre | Plage Freq (Hz) | Max Emergence (TTNR)"
