...
object PdfReportGenerator {
    ...
    /**
     * Word-wraps the operator's comments, **preserving their line breaks** [A4, plan 4.5].
     *
     * The old code split on `" "` and `"\n"` in one call, so every newline was consumed
     * before it could be honoured: a comment written as a list arrived on the customer's
     * report as one flattened paragraph. Pure (the width measurement is injected) so the
     * behaviour is unit-testable without a Canvas.
     */
    internal fun wrapComment(
        text: String,
        maxWidth: Float,
        maxLines: Int,
    ...
    fun generateReport(
        ...
        val methodStr =
            when (kinematicsConfig.inputMode) {
        ...
        // [U7, plan 4.5] Traceability: when, which build, which source, which speed
        // reconstruction. A deliverable that carries numbers but not their provenance
        // cannot be defended once it leaves the room.
        canvas.drawText(
            "Édité le ${stamp.formattedTimestamp()} · NVH Spectro v${stamp.appVersion}",
            col2X,
            infoBoxTop + 45f,
            textPaint,
        )

        currentY = infoBoxBottom + 14f
        val stampPaint =
            Paint(textPaint).apply {
                textSize = 7.5f
                color = Color.DKGRAY
            }
        listOf(stamp.sourceLine, stamp.analysisLine, stamp.speedLine).forEach { line ->
            canvas.drawText(line, margin, currentY, stampPaint)
            currentY += 9f
        }
        currentY += 12f
            ...
            if (bitmap != null) {
                ...
                if (drawBrilliance) {
                    ...
                    trackedOrders.forEach { order ->
                        // [U7, plan 4.5] The order's OWN colour — the same value the screen
                        // drew. The report used a golden-angle HSV sequence instead, so the
                        // same order was a different colour on screen and on paper, with no
                        // legend tying them together.
                        pathPaint.color = order.color.toArgbInt()
        ...
        drawColormapBox(canvas, absoluteBitmap, leftX, currentY, mapWidth, mapHeight, "Colormap absolue (dBFS)", false)
        drawColormapBox(
            canvas,
            ttnrBitmap,
            rightX,
            ...
            mapWidth,
            mapHeight,
            ReportStamp.EMERGENCE_METRIC_NAME,
            false,
        )
        ...
        val scale =
            if (requiredListHeight > maxAvailableHeightForListAndMaps) {
        ...
        if (trackedOrders.isEmpty()) {
            ...
            trackedOrders.forEach { order ->
                ...
                canvas.drawText("${order.name} (émergence max : $eMax dB)", txtX, listY, listBoldPaint)
                listY += 12f * scale
        ...
        val commentsBottom = currentY + commentsBoxHeight - 10f
        val maxLines = (((commentsBottom - cy) / 12f).toInt() + 1).coerceAtLeast(1)

        wrapComment(comments, maxTextWidth, maxLines) { textPaint.measureText(it) }
            .forEach { line ->
                canvas.drawText(line, leftX + 5f, cy, textPaint)
                cy += 12f
            }

        // [D1, decision D5] The metric is named honestly and defined on the page itself.
        val notePaint =
            Paint(textPaint).apply {
                textSize = 6.5f
                color = Color.DKGRAY
            }
        canvas.drawText(ReportStamp.EMERGENCE_METRIC_NOTE, margin, 842f - 12f, notePaint)
