...
new_func = '''    private fun recalculateSmartPath() {
        ...
        // Rayon de recherche tres large pour chasser l'harmonique meme si le point est pose loin
        val maxSearchRadius = 25 
        ...
        // Ligne directrice : Interpolation LINEAIRE (evite les marches d'escalier du Cosinus)
        ...
        // Prominence : Comparaison du pic par rapport au vrai bruit de fond local (en excluant le pic lui-meme)
        fun getProminence(f: Int, b: Int): Float {
            var sum = 0f
            var count = 0
            for (i in -5..5) {
                if (Math.abs(i) <= 1) continue // Exclut le pic central pour ne pas fausser la moyenne du bruit
                val neighbor = b + i
        ...
        for (f in 0 until numFrames) {
            ...
            for (b in minBin..maxBin) {
                ...
                // Penalite lineaire douce pour permettre a l'algorithme d'aller chercher une harmonique eloignee
                val distToExpected = Math.abs(b.toFloat() - expectedBinF)
                val guidePenalty = 1.0f * distToExpected
                
                // Poids forts sur la prominence (pics)
                val nodeScore = (3.0f * prominence) + (0.2f * energyDb) - guidePenalty
                ...
                if (f == 0) {
                    ...
                    for (prevB in prevMin..prevMax) {
                        ...
                        if (prevScore > -Float.MAX_VALUE) {
                            ...
                            // Penalite quadratique sur la DEVIATION de pente (pas sur le saut lui-meme)
                            // Ca permet de suivre des pentes raides sans penalite tant qu'on suit la direction de l'utilisateur !
                            val jumpPenalty = 2.0f * jumpDeviation * jumpDeviation
...
print('Updated MainViewModel.kt with advanced slope tracking')
