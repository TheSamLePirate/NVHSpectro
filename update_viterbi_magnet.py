...
new_func = '''    private fun recalculateSmartPath() {
        ...
        // Rayon tres large : le point utilisateur est un guide, l'algo peut chercher l'harmo tres loin (environ 800 Hz de marge)
        val maxSearchRadius = 150 
        ...
        // Energie en dB (deja calcule par FFTProcessor)
        ...
        // Prominence : Detection d'une crete par rapport a un bruit de fond large
        fun getProminence(f: Int, b: Int): Float {
            var sum = 0f
            var count = 0
            // On regarde sur une large bande (+/- 10 bins = ~50 Hz)
            for (i in -10..10) {
                // On exclut la crete elle-meme et ses epaules immediates (+/- 2 bins = ~25 Hz)
        ...
        for (f in 0 until numFrames) {
            ...
            for (b in minBin..maxBin) {
                ...
                // Penalite tres faible pour s'eloigner du guide. L'algo est libre de chasser l'harmo loin !
                val distToExpected = Math.abs(b.toFloat() - expectedBinF)
                val guidePenalty = 0.5f * distToExpected
                
                // AIMANT A HARMONIQUES : On recompense COLOSSALEMENT la prominence (x20.0)
                val nodeScore = (20.0f * prominence) + (1.0f * energyDb) - guidePenalty
                ...
                if (f == 0) {
                    ...
                    for (prevB in prevMin..prevMax) {
                        ...
                        if (prevScore > -Float.MAX_VALUE) {
                            ...
                            // Penalite permissive pour laisser l'algo suivre des pentes tres raides (harmoniques fuyantes)
                            // Un mix lineaire (pour accepter la pente) et petit quadratique (pour eviter les zigzags aleatoires)
                            val jumpPenalty = 2.0f * jumpDeviation + 0.2f * jumpDeviation * jumpDeviation
...
print('Updated MainViewModel.kt with magnet tracking')
